...
internal constructor(obj: IEGenObject) : AutoCloseable {
    companion object {
        ...
        public fun iegenDowncast(id: Long, owner: Boolean = false): Fruit {
            val idType = jGettypebyid(id)
            when (idType) {
                "iegen::example::Apple" -> return Apple(IEGenObject(id, owner))
                "iegen::example::Pineapple" -> return Pineapple(IEGenObject(id, owner))
    ...
    open var applesWithPineapples: List<Fruit>
        get() {
            ...
            for (value_result in result) {
                ...
                jdktokotlinvalue_result = Fruit.iegenDowncast(value_result)
                jdktokotlinresult.add(jdktokotlinvalue_result)
            }
