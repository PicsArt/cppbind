...
internal constructor(_id: Long) : AutoCloseable {
    ...
    open val fooBar: Pair<Foo, Bar>
        get() {
            val err = ErrorObj()
            val result = jPairFooBar(getObjId(), err)
            if (err.typeId != 0) {
                when (err.typeId) {
                    1 -> {
    ...
    open val fooBarPair: Pair<Foo, Bar>
        get() {
            val err = ErrorObj()
            val result = jPairwithtypeFooBar(getObjId(), err)
            if (err.typeId != 0) {
                when (err.typeId) {
                    1 -> {
                        val excObj = StdException(err.ptrId)
    ...
    open val barBarPair: Pair<Bar, Bar>
        get() {
            val err = ErrorObj()
            val result = jPairwithtypeBarBar(getObjId(), err)
            if (err.typeId != 0) {
                when (err.typeId) {
                    1 -> {
                        val excObj = StdException(err.ptrId)
    ...
    ///// External wrapper functions ////////////
    private external fun jPairFooBar(id: Long, __err_obj__: ErrorObj): Pair<Long, Long>
    private external fun jPairwithtypeFooBar(id: Long, __err_obj__: ErrorObj): Pair<Long, Long>
    private external fun jPairwithtypeBarBar(id: Long, __err_obj__: ErrorObj): Pair<Long, Long>
    private external fun jValue(id: Long): Int
    private external fun jSetvalue(id: Long, value: Int): Unit
    private external fun jSet_this(id: Long, self: Any): Unit
    private external fun jFinalize(id: Long): Unit
