...
import com.examples.exception_helpers.*import com.examples.simple.Root
...
interface IAddressableRoot {
    ...
    fun absPath(): String {
        val err = ErrorObj()
        val result = AddressableHelperRoot.jAbspath(getObjId(), err)
        if (err.typeId != 0) {
            when (err.typeId) {
                1 -> {
...
class AddressableHelperRoot {
    companion object {
        @JvmStatic
        external fun jAbspath(id: Long, __err__obj__: ErrorObj): String
    }
}
