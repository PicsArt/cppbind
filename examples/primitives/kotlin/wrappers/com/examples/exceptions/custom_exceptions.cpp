...
iegen::example::SimpleBaseException* downcast_IegenExample_SimpleBaseException(jobjectid id) {
    auto iegen_obj_id = reinterpret_cast<IEGenCObject*>(id);
    auto iegen_obj_id_ptr = iegen_obj_id ? iegen_obj_id->ptr : nullptr;
    if (iegen_obj_id_ptr) {
        if (strcmp(iegen_obj_id->type, IEGEN_IEGEN_EXAMPLE_SIMPLEBASEEXCEPTION) == 0)
...
extern "C" JNIEXPORT jint Java_com_examples_exceptions_SimpleBaseException_jErrnum(JNIEnv* env, jobject obj, jobjectid id){
    ...
    validateID(id);
    iegen::example::SimpleBaseException* jnitocxxid = downcast_IegenExample_SimpleBaseException(id);
