...
const char* IEGEN_IEGEN_EXAMPLE_DATE = "iegen::example::Date";
...
extern "C" JNIEXPORT void Java_com_examples_inheritance_Date_jFinalize(JNIEnv* env, jobject obj, jobjectid id){
    ...
    if (strcmp(iegen_obj_id->type, IEGEN_IEGEN_EXAMPLE_DATE) == 0)
...
std::shared_ptr<iegen::example::Date> recover_obj_from_IegenExample_Date(jobjectid id) {
    auto iegen_obj_id = reinterpret_cast<IEGenCObject*>(id);
    auto iegen_obj_id_ptr = iegen_obj_id ? iegen_obj_id->ptr : nullptr;
    if (iegen_obj_id_ptr) {
        if (strcmp(iegen_obj_id->type, IEGEN_IEGEN_EXAMPLE_DATE) == 0)
...
extern "C" JNIEXPORT jobjectid Java_com_examples_inheritance_Date_jConstructor(JNIEnv* env, jobject obj, jint d, jint m, jint y){
    ...
    try {
        ...
        return reinterpret_cast<jlong>(new IEGenCObject {strdup(IEGEN_IEGEN_EXAMPLE_DATE), new std::shared_ptr<iegen::example::Date>(this_object)});
    }
...
extern "C" JNIEXPORT jstring Java_com_examples_inheritance_Date_jValue(JNIEnv* env, jobject obj, jobjectid id){
    ...
    jnitocxxid = recover_obj_from_IegenExample_Date(id);
