...
public class Exc  {
     ...
     * 
     */
    public static func getByKey(m: Dictionary<Int, Int>, k: Int) throws -> Int {
        ...
        let swift_to_sc_k = CInt(k)
        var err = ErrorObj()
        let result = _func_Exc_getByKey(swift_to_sc_m, swift_to_sc_k, &err);
        let sc_to_swift_result = Int(result)
        let err_type = Int(err.err_type)
        if (err_type != 0) {
            switch(err_type) {
