...
public class MiscExc  {
    ...
    public static func returnInteger(do_throw: Bool) throws -> Integer {
        ...
        if (err_type != 0) {
            switch(err_type) {
                case(1):
                    throw StdOutOfRange(CDataObj(index: 0, ptr: err.err_ptr), true)
                case(2):
    ...
    public static func raiseErrorByType(err_type: String) throws -> Void {
        ...
        if (err_type != 0) {
            switch(err_type) {
                case(1):
                    throw StdRuntimeError(CDataObj(index: 0, ptr: err.err_ptr), true)
                case(2):
                    throw FileError(CDataObj(index: 0, ptr: err.err_ptr), true)
                case(3):
                    throw SystemError(CDataObj(index: 0, ptr: err.err_ptr), true)
                case(4):
                    throw SimpleChildException(CDataObj(index: 0, ptr: err.err_ptr), true)
                case(5):
                    throw SimpleBaseException(CDataObj(index: 0, ptr: err.err_ptr), true)
                case(6):
                    throw StdException(CDataObj(index: 0, ptr: err.err_ptr), true)
                default:
                    ExceptionHandler.handleUncaughtException("Uncaught Exception")
            }
        }
