...
public class ParallelogramImpl: Parallelogram {
  ...
  class var iegenCxxTypeName : String { return "iegen::example::Parallelogram" }

  class func iegenConstructObject(_ iegenObj: IEGenCObject, _ owner: Bool = false) -> Parallelogram {
    let typeName = String(cString: iegenObj.type)
    switch(typeName) {
    case(RectangleImpl.iegenCxxTypeName):
      return RectangleImpl(iegenObj, owner)
    case(RhombusFigureImpl.iegenCxxTypeName):
