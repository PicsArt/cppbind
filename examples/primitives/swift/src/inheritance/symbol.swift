/**
 * ,--.,------. ,----.   ,------.,--.  ,--. 
 * |  ||  .---''  .-.|   |  .---'|  ,'.|  | 
 * |  ||  `--, |  | .---.|  `--, |  |' '  | 
 * |  ||  `---.'  '--'  ||  `---.|  | `   | 
 * `--'`------' `------' `------'`--'  `--' 
 * 
 * This file is generated by iegen on 01/14/2022-08:55.
 * Please do not change it manually.
 */

import CWrapper
import Foundation

// public protocol Symbol {
//   var cself: CObject { get }
//
//   func typeName() -> String
// }
//
// extension Symbol {
//   public func typeName() -> String {
//
//     var err = ErrorObj()
//     let result = _func_Symbol_typeName(cself, &err)
//     let errorType = Int(err.err_type)
//     if errorType != 0 {
//       switch errorType {
//       case (1):
//         let excObj = StdException(CObject(type: strdup("std::exception"), ptr: err.err_ptr), true)
//         ExceptionHandler.handleUncaughtException(excObj.what())
//       default:
//         ExceptionHandler.handleUncaughtException("Uncaught Exception")
//       }
//     }
//     let sctoswiftresult = String(cString: result)
//     defer {_func_Number_typeName
//       result.deallocate()
//     }
//     return sctoswiftresult
//   }
//
// }
//
// public class SymbolImpl: Symbol {
//   public let cself: CObject
//   public let owner: Bool
//
//   // internal main initializer
//   internal required init(_ _cself: CObject, _ _owner: Bool = false) {
//     self.cself = _cself
//     self.owner = _owner
//   }
//
//   deinit {
//     release_SymbolImpl(cself, owner)
//   }
//
//   public convenience init() {
//     var err = ErrorObj()
//     self.init(create_Symbol(&err), true)
//     let errorType = Int(err.err_type)
//     if errorType != 0 {
//       switch errorType {
//       case (1):
//         let excObj = StdException(CObject(type: strdup("std::exception"), ptr: err.err_ptr), true)
//         ExceptionHandler.handleUncaughtException(excObj.what())
//       default:
//         ExceptionHandler.handleUncaughtException("Uncaught Exception")
//       }
//     }
//   }
//
// }

public protocol Number {
  var cself: CObject { get }
  func typeName() -> String
}

extension Number {
  public func typeName() -> String {

    var err = ErrorObj()
    let result = _func_Number_typeName(cself, &err)
    let errorType = Int(err.err_type)
    if errorType != 0 {
      switch errorType {
      case (1):
        let excObj = StdException(CObject(type: strdup("std::exception"), ptr: err.err_ptr), true)
        ExceptionHandler.handleUncaughtException(excObj.what())
      default:
        ExceptionHandler.handleUncaughtException("Uncaught Exception")
      }
    }
    let sctoswiftresult = String(cString: result)
    defer {
      result.deallocate()
    }
    return sctoswiftresult
  }

}

public class NumberImpl: Number {
  public let cself: CObject
  public let owner: Bool

  // internal main initializer
  internal required init(_ _cself: CObject, _ _owner: Bool = false) {
    self.cself = _cself
    self.owner = _owner
  }

  deinit {
    release_NumberImpl(cself, owner)
  }

  public convenience init() {
    var err = ErrorObj()
    self.init(create_Number(&err), true)
    let errorType = Int(err.err_type)
    if errorType != 0 {
      switch errorType {
      case (1):
        let excObj = StdException(CObject(type: strdup("std::exception"), ptr: err.err_ptr), true)
        ExceptionHandler.handleUncaughtException(excObj.what())
      default:
        ExceptionHandler.handleUncaughtException("Uncaught Exception")
      }
    }
  }

}

public class Text {

  public let cself: CObject
  public let owner: Bool

  // internal main initializer
  internal required init(_ _cself: CObject, _ _owner: Bool = false) {
    self.cself = _cself
    self.owner = _owner
  }

  deinit {
    release_Text(cself, owner)
  }

  public convenience init() {
    var err = ErrorObj()
    self.init(create_Text(&err), true)
    let errorType = Int(err.err_type)
    if errorType != 0 {
      switch errorType {
      case (1):
        let excObj = StdException(CObject(type: strdup("std::exception"), ptr: err.err_ptr), true)
        ExceptionHandler.handleUncaughtException(excObj.what())
      default:
        ExceptionHandler.handleUncaughtException("Uncaught Exception")
      }
    }
  }

  public func typeName() -> String {

    var err = ErrorObj()
    let result = _func_Text_typeName(cself, &err)
    let errorType = Int(err.err_type)
    if errorType != 0 {
      switch errorType {
      case (1):
        let excObj = StdException(CObject(type: strdup("std::exception"), ptr: err.err_ptr), true)
        ExceptionHandler.handleUncaughtException(excObj.what())
      default:
        ExceptionHandler.handleUncaughtException("Uncaught Exception")
      }
    }
    let sctoswiftresult = String(cString: result)
    defer {
      result.deallocate()
    }
    return sctoswiftresult
  }

}

public class Digit: Text, Number {
  public convenience init() {
    var err = ErrorObj()
    self.init(create_Digit(&err), true)
    let errorType = Int(err.err_type)
    if errorType != 0 {
      switch errorType {
      case (1):
        let excObj = StdException(CObject(type: strdup("std::exception"), ptr: err.err_ptr), true)
        ExceptionHandler.handleUncaughtException(excObj.what())
      default:
        ExceptionHandler.handleUncaughtException("Uncaught Exception")
      }
    }
  }

  public override func typeName() -> String {

    var err = ErrorObj()
    let result = _func_Digit_typeName(cself, &err)
    let errorType = Int(err.err_type)
    if errorType != 0 {
      switch errorType {
      case (1):
        let excObj = StdException(CObject(type: strdup("std::exception"), ptr: err.err_ptr), true)
        ExceptionHandler.handleUncaughtException(excObj.what())
      default:
        ExceptionHandler.handleUncaughtException("Uncaught Exception")
      }
    }
    let sctoswiftresult = String(cString: result)
    defer {
      result.deallocate()
    }
    return sctoswiftresult
  }

}

public class SymbolUsage {

  public let cself: CObject
  public let owner: Bool

  // internal main initializer
  internal required init(_ _cself: CObject, _ _owner: Bool = false) {
    self.cself = _cself
    self.owner = _owner
  }

  deinit {
    release_SymbolUsage(cself, owner)
  }

  public convenience init() {
    var err = ErrorObj()
    self.init(create_SymbolUsage(&err), true)
    let errorType = Int(err.err_type)
    if errorType != 0 {
      switch errorType {
      case (1):
        let excObj = StdException(CObject(type: strdup("std::exception"), ptr: err.err_ptr), true)
        ExceptionHandler.handleUncaughtException(excObj.what())
      default:
        ExceptionHandler.handleUncaughtException("Uncaught Exception")
      }
    }
  }

  public convenience init(d: Digit) {
    let swifttoscd = d.cself
    var err = ErrorObj()
    self.init(create_SymbolUsage_1(swifttoscd, &err), true)
    let errorType = Int(err.err_type)
    if errorType != 0 {
      switch errorType {
      case (1):
        let excObj = StdException(CObject(type: strdup("std::exception"), ptr: err.err_ptr), true)
        ExceptionHandler.handleUncaughtException(excObj.what())
      default:
        ExceptionHandler.handleUncaughtException("Uncaught Exception")
      }
    }
  }

  public func getTextType(t: Text) -> String {

    let swifttosct = t.cself
    var err = ErrorObj()
    let result = _func_SymbolUsage_getTextType(cself, swifttosct, &err)
    let errorType = Int(err.err_type)
    if errorType != 0 {
      switch errorType {
      case (1):
        let excObj = StdException(CObject(type: strdup("std::exception"), ptr: err.err_ptr), true)
        ExceptionHandler.handleUncaughtException(excObj.what())
      default:
        ExceptionHandler.handleUncaughtException("Uncaught Exception")
      }
    }
    let sctoswiftresult = String(cString: result)
    defer {
      result.deallocate()
    }
    return sctoswiftresult
  }

  public func getNumberType(n: Number) -> String {

    let swifttoscn = n.cself
    var err = ErrorObj()
    let result = _func_SymbolUsage_getNumberType(cself, swifttoscn, &err)
    let errorType = Int(err.err_type)
    if errorType != 0 {
      switch errorType {
      case (1):
        let excObj = StdException(CObject(type: strdup("std::exception"), ptr: err.err_ptr), true)
        ExceptionHandler.handleUncaughtException(excObj.what())
      default:
        ExceptionHandler.handleUncaughtException("Uncaught Exception")
      }
    }
    let sctoswiftresult = String(cString: result)
    defer {
      result.deallocate()
    }
    return sctoswiftresult
  }

  public func getTextPtr() -> Text {

    var err = ErrorObj()
    let result = _func_SymbolUsage_getTextPtr(cself, &err)
    let errorType = Int(err.err_type)
    if errorType != 0 {
      switch errorType {
      case (1):
        let excObj = StdException(CObject(type: strdup("std::exception"), ptr: err.err_ptr), true)
        ExceptionHandler.handleUncaughtException(excObj.what())
      default:
        ExceptionHandler.handleUncaughtException("Uncaught Exception")
      }
    }
    var sctoswiftresult: Text
    let resultIndex = String(cString: result.type)
    switch(resultIndex) {
    case("iegen::example::Digit"):
      sctoswiftresult = Digit(result)
    default:
      sctoswiftresult = Text(result)
    }
    return sctoswiftresult
  }

  public func getNumberPtr() -> Number {

    var err = ErrorObj()
    let result = _func_SymbolUsage_getNumberPtr(cself, &err)
    let errorType = Int(err.err_type)
    if errorType != 0 {
      switch errorType {
      case (1):
        let excObj = StdException(CObject(type: strdup("std::exception"), ptr: err.err_ptr), true)
        ExceptionHandler.handleUncaughtException(excObj.what())
      default:
        ExceptionHandler.handleUncaughtException("Uncaught Exception")
      }
    }
    var sctoswiftresult: Number
    let resultIndex = String(cString: result.type)
    switch(resultIndex) {
    case("iegen::example::Digit"):
      sctoswiftresult = Digit(result)
    default:
      sctoswiftresult = NumberImpl(result)
    }
    return sctoswiftresult
  }

}
