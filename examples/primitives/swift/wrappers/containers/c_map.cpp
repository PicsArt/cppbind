...
int _prop_get_MapItem_value(CObject cself){
    iegen::example::MapItem* ctocxxcself_ptr = nullptr;
    if (cself.ptr) {
        if (strcmp(cself.type, "iegen::example::MapItem") == 0)
            ctocxxcself_ptr = static_cast<iegen::example::MapItem*>(cself.ptr);
        else {
...
void _func_MapExamples_addStringPair(CObject cself, CDataPair info, ErrorObj* _Nonnull err){
    ...
    iegen::example::MapExamples* ctocxxcself_ptr = nullptr;
    if (cself.ptr) {
        if (strcmp(cself.type, "iegen::example::MapExamples") == 0)
            ctocxxcself_ptr = static_cast<iegen::example::MapExamples*>(cself.ptr);
        else {
            std::cerr << "Unexpected object type: " << cself.type << std::endl;
            exit(1);
        }
...
void _func_MapExamples_addMixedMap(CObject cself, CDataMap info, ErrorObj* _Nonnull err){
    ...
    for (size_t _i_info = 0; _i_info < info.size; ++_i_info) {
        ...
        iegen::example::MapItem* ctocxxvtmp_info_ptr = nullptr;
        if (vtmp_info.ptr) {
            if (strcmp(vtmp_info.type, "iegen::example::MapItem") == 0)
                ctocxxvtmp_info_ptr = static_cast<iegen::example::MapItem*>(vtmp_info.ptr);
            else {
                std::cerr << "Unexpected object type: " << vtmp_info.type << std::endl;
                exit(1);
            }
        }
        auto& ctocxxvtmp_info = *ctocxxvtmp_info_ptr;
        ctocxxinfo.insert({ ctocxxktmp_info, ctocxxvtmp_info });
    }
    ...
    try {
        ...
        for (auto& value_result : result) {
            ...
            const auto& val_result = value_result.second;
            {
                ...
                CObject cxxtocval_result = {type_val_result, value_ptr_val_result};
                _k_data_cxxtocresult[_i_result] = cxxtockey_result;
                _v_data_cxxtocresult[_i_result] = cxxtocval_result;
                _i_result += 1;
            }
