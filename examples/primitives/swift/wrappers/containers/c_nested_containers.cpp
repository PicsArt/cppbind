...
void _func_NestedExamples_setComplexNestedIntVector(void* _Nonnull cself, CDataArray v, ErrorObj* _Nonnull err){
    std::vector<std::vector<std::vector<int> > > c_to_cxx_v;

    auto data_v = reinterpret_cast<CDataArray*>(v.data);

    for (size_t _i_v = 0; _i_v < v.size; ++_i_v) {
        auto& value_v = data_v[_i_v];
        std::vector<std::vector<int> > c_to_cxx_value_v;

        auto data_value_v = reinterpret_cast<CDataArray*>(value_v.data);

        for (size_t _i_value_v = 0; _i_value_v < value_v.size; ++_i_value_v) {
            auto& value_value_v = data_value_v[_i_value_v];
            std::vector<int> c_to_cxx_value_value_v;

            auto data_value_value_v = reinterpret_cast<int*>(value_value_v.data);
            ...
            for (size_t _i_value_value_v = 0; _i_value_value_v < value_value_v.size; ++_i_value_value_v) {
                ...
                c_to_cxx_value_value_v.emplace_back(value_value_value_v);
            }
    ...
    auto c_to_cxx_cself = dynamic_cast<iegen::example::NestedExamples*>(static_cast<iegen::example::NestedExamples*>(cself));
    try {
        c_to_cxx_cself->setComplexNestedIntVector(c_to_cxx_v);
    }
    catch (const std::exception& e) {
        err->err_type = 1;
        err->err_ptr = new std::exception(e);
...
CDataArray _func_NestedExamples_getComplexNestedIntVector(void* _Nonnull cself, ErrorObj* _Nonnull err){
    auto c_to_cxx_cself = dynamic_cast<iegen::example::NestedExamples*>(static_cast<iegen::example::NestedExamples*>(cself));
    try {
      const auto& result = c_to_cxx_cself->getComplexNestedIntVector();
      auto _data_cxx_to_c_result = new CDataArray [result.size()];
    CDataArray cxx_to_c_result = { _data_cxx_to_c_result, (long long)result.size() };
    for (int _i_result = 0; _i_result < result.size(); ++_i_result) {
      auto& value_result = result[_i_result];
      auto _data_cxx_to_c_value_result = new CDataArray [value_result.size()];
        ...
        for (int _i_value_result = 0; _i_value_result < value_result.size(); ++_i_value_result) {
          auto& value_value_result = value_result[_i_value_result];
          auto _data_cxx_to_c_value_value_result = new int [value_value_result.size()];
            ...
            for (int _i_value_value_result = 0; _i_value_value_result < value_value_result.size(); ++_i_value_value_result) {
              ...
              _data_cxx_to_c_value_value_result[_i_value_value_result] = value_value_value_result;
            }
