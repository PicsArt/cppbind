...
CppBindCDataArray _func_CppbindExample_getVectorColor(CppBindCDataArray c, CppBindCObject* _Nonnull cppbind_err) {
    using namespace cppbind::example;
    std::vector<Color> ctocxxc;

    auto data_c = reinterpret_cast<int*>(c.data);

    for (size_t _i_c = 0; _i_c < c.size; ++_i_c) {
        auto& value_c = data_c[_i_c];
        auto ctocxxvalue_c = static_cast<cppbind::example::Color>(value_c);
        ctocxxc.emplace_back(ctocxxvalue_c);
    }
    try {
        decltype(auto) result = cppbind::example::getVectorColor(ctocxxc);
        ...
        for (int _i_result = 0; _i_result < result.size(); ++_i_result) {
            ...
            int cxxtocvalue_result = static_cast<int>(value_result);
