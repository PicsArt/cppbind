...
char* _Nonnull _func_SymbolUsage_getSignType(CObject cself, CObject n, ErrorObj* _Nonnull err){
    ...
    iegen::example::Sign* ctocxxn_ptr = nullptr;
    if (n.ptr) {
        if (strcmp(n.type, "iegen::example::Sign") == 0)
            ctocxxn_ptr = static_cast<iegen::example::Sign*>(n.ptr);
        else if (strcmp(n.type, "iegen::example::Digit") == 0)
            ctocxxn_ptr = static_cast<iegen::example::Digit*>(n.ptr);
        else {
            std::cerr << "Unexpected object type: " << n.type << std::endl;
            exit(1);
        }
    }
    auto ctocxxn = ctocxxn_ptr;
    ...
    auto ctocxxcself = ctocxxcself_ptr;
    try {
        const auto& result = ctocxxcself->getSignType(ctocxxn);
        auto cxxtocresult = strdup(result.data());
        return cxxtocresult;
    }
    catch (const std::exception& e) {
