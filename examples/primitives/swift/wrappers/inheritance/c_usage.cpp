 ...
 * `--'`------' `------' `------'`--'  `--' 
 * 
 * This file is generated by iegen on 12/01/2021-14:37.
...
CDataObj _prop_get_GeometricFigure_nullableParallelogram(CDataObj cself){
    auto c_to_cxx_cself = static_cast<iegen::example::GeometricFigure*>(cself.ptr);
...
void _prop_set_GeometricFigure_nullableParallelogram(CDataObj cself, CDataObj p){
...
CDataObj _prop_get_MyVehicle_vehicle(CDataObj cself){
    ...
    if (result) {
        ...
        int status = 0;
        const char * name = typeid(arg_pointee_result).name();
        char* res = abi::__cxa_demangle(name, NULL, NULL, &status);
        std::string demangled_result = (status==0) ? res : name;
        free(res);
        if (demangled_result == "iegen::example::Bicycle") {
            code_result = 1;
        } 
