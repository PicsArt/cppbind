...
IEGenCObject create_IegenExample_MyVehicle(IEGenCObject v, IEGenCObject cv, IEGenCObject* _Nonnull err) {
    ...
    if (strcmp(cv.type, "iegen::example::Vehicle") == 0)
        ctocxxcv = *static_cast<std::shared_ptr<const iegen::example::Vehicle>*>(cv.ptr);
    else if (strcmp(cv.type, "iegen::example::Bicycle") == 0)
        ctocxxcv = *static_cast<std::shared_ptr<iegen::example::Bicycle>*>(cv.ptr);
    else {
        std::cerr << "Unexpected object type: " << cv.type << std::endl;
        exit(1);
    }
    try {
...
void _prop_set_IegenExample_MyVehicle_constVehicle(IEGenCObject cself, IEGenCObject v, IEGenCObject* _Nonnull err) {
    ...
    if (v.ptr) {
        if (strcmp(v.type, "iegen::example::Vehicle") == 0)
            ctocxxv = *static_cast<std::shared_ptr<const iegen::example::Vehicle>*>(v.ptr);
        else if (strcmp(v.type, "iegen::example::Bicycle") == 0)
            ctocxxv = *static_cast<std::shared_ptr<iegen::example::Bicycle>*>(v.ptr);
        else {
            std::cerr << "Unexpected object type: " << v.type << std::endl;
