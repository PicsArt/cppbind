...
void release_Task(void* _Nonnull cself) {
    delete static_cast<std::shared_ptr<iegen::example::Task>*>(cself);
}
void* _Nonnull create_Task(char* _Nonnull title){
    ...
    return new std::shared_ptr<iegen::example::Task>(this_object);
}
char* _Nonnull _prop_get_Task_title(void* _Nonnull cself){
    auto c_to_cxx_cself = std::dynamic_pointer_cast<iegen::example::Task>(
              *static_cast<std::shared_ptr<iegen::example::Task>*>(cself)
              );
