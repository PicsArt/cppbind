...
This file is generated by iegen on 02/07/2022-05:52.
...
class Function:
    ...
    def is_property(self):
        if self.classname is None:
            return False
        # on pybind side we do not have nested types using the last part of iegen's type full name
    ...
    def pybind_module(self):
        ...
        module = getattr(importlib.import_module(self.original_function.__module__), module_name, None)
        if module is None:
            raise ImportError(f'Could not load corresponding pybind module for: {self.original_function.__module__}')
        return module
...
class bind:
        ...
        def _decorator(*args, **kwargs):
            ...
            if inspect.isclass(instance):
                ...
                if not hasattr(instance, 'originals'):
                    ...
                    cls = self.__load_cls(instance.__module__, instance.__qualname__)
                else:
    ...
    def __call__(self, *args, **kwargs):
        ...
        if args and inspect.isclass(args[0]):
            ...
            if not hasattr(args[0], 'originals'):
                ...
                cls = self.__load_cls(args[0].__module__, args[0].__qualname__)
            else:
        ...
        if self.fn.is_function:
            # case for global functions
            # no overloading for global function
        ...
        if self.fn.is_property:
            # case of properties, e.g decorated with @property
            # get the non pybind class
            cls = self.__load_cls(args[0].__module__, args[0].__class__.__qualname__)

            original = cls.originals[self.fn.name]
            if isinstance(original, property):
                if len(args) == 2:
                    # setter
                    converted_args, _ = self.__validate_and_convert_args_kwargs(self.fn, args[1])
    ...
    @staticmethod
    def __load_cls(module_name, qualname):
        # if the type is nested load it's parent first
