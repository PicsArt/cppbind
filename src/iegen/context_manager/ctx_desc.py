...
class ContextDescriptor:
    ...
    def __init__(self, parser_config):
        ...
        self.api_ctx_map = ContextDescriptor.build_api_ctx_map(parser_config)
        self.attributes = ContextDescriptor.resolve_attr_aliases(self.get_var_def())
    ...
    @staticmethod
    def build_api_ctx_map(parser_config):
        ...
        for current_file in list(files):
            ...
            ContextDescriptor.update_api_ctx_map(attrs, current_file, api_ctx_map)

        return api_ctx_map

    @classmethod
    def update_api_ctx_map(cls, attrs, current_file, api_ctx_map):
            ...
            try:
                if cls.VARS_RULE_KEY in src_dict:
                    flat_key = cls._get_key(current_file=current_file,
                                            src_dict=src_dict,
                                            ancestors=ancestors)
                    if flat_key in api_ctx_map:
                        raise YamlKeyDuplicationError(
                            f"Definition with duplicate '{flat_key}' key in {current_file},\n"
                            f"which already has been previously defined in {api_ctx_map[flat_key].file}")
                    api_ctx_map[flat_key] = SimpleNamespace(attr=src_dict[cls.VARS_RULE_KEY],
                                                            file=current_file)
        ...
        if cls.ROOT_SECTION_KEY in attrs:
            if RootNode.ROOT_KEY in api_ctx_map:
                raise YamlKeyDuplicationError(f"Redefinition of '{cls.ROOT_SECTION_KEY}' section in {current_file} file, "
                                              ...
                                              f"It was previously defined in {api_ctx_map[RootNode.ROOT_KEY].file} file.")
            api_ctx_map[RootNode.ROOT_KEY] = SimpleNamespace(attr=attrs[cls.ROOT_SECTION_KEY],
                                                             file=current_file)
        ...
        if cls.VAR_DEF_SECTION_KEY in attrs:
            api_ctx_map[cls.VAR_DEF_SECTION_KEY] = attrs[cls.VAR_DEF_SECTION_KEY]
    ...
    def get_var_def(self):
        return self.api_ctx_map.get(ContextDescriptor.VAR_DEF_SECTION_KEY, {})
