...
def test_parser_with_dir_api(clang_config):
    ...
    with patch('iegen.context_manager.ctx_desc.default_config') as config_cfg_mock:
        config_cfg_mock.application.context_def_glob = os.path.join(api_rules_dir, dir_example_folder, '*.yaml')
...
@patch('os.getcwd', lambda: SCRIPT_DIR)
def test_parser_with_file_api(clang_config, parser_config):
    clang_cfg = copy.deepcopy(clang_config)
    ...
    with patch('iegen.context_manager.ctx_desc.default_config') as config_cfg_mock:
        config_cfg_mock.application.context_def_glob = os.path.join(api_rules_dir, file_example_folder, '*.yaml')
